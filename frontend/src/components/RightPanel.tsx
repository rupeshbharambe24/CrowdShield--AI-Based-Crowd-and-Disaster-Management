...
import { AlertTriangle, Users, Building, Clock, Check } from 'lucide-react';
...
const AnimatedCounter: React.FC<{ value?: number; label: string; critical?: boolean }> = ({ 
  ...
  return (
    ...
    >
      <motion.div
        className={`text-3xl font-bold mb-2 ${critical ? 'text-crowd-critical' : 'text-crowd-electric'}`}
        key={value}
...
export const RightPanel: React.FC = () => {
  ...
  const getSeverityClass = (severity: string) => {
    switch (severity) {
      case 'critical': return 'border-l-4 border-crowd-critical bg-crowd-critical/10';
      case 'high': return 'border-l-4 border-crowd-warning bg-crowd-warning/10';
      case 'medium': return 'border-l-4 border-crowd-electric bg-crowd-electric/10';
      default: return 'border-l-4 border-crowd-safe bg-crowd-safe/10';
    }
  };

  const getSeverityIcon = (severity: string) => {
    switch (severity) {
      case 'critical': return <AlertTriangle className="w-4 h-4 text-crowd-critical" />;
      case 'high': return <AlertTriangle className="w-4 h-4 text-crowd-warning" />;
      default: return <Clock className="w-4 h-4 text-crowd-electric" />;
    }
  };
  ...
  return (
    ...
    >
      ...
      <div className="p-4 border-b border-white/10">
        <h3 className="text-lg font-semibold text-crowd-electric mb-4">Key Performance Indicators</h3>
